...
public class LoginController {
	...
	public String userInfo(Model model, Principal principal) throws Exception {
		...
		/**
		 * int employeeID =userDao.getEmployeeID(userName); logger.info("EmployeeID is"
		 * + employeeID); UserBean usrB = new UserBean(); usrB =
		 * userDao.getUser(userName); logger.info("ID in USER table is" + usrB.getId());
		 * UserRoleBean usrRB = new UserRoleBean();
		 * usrRB=userRoleDAO.getUserRoleByidUser(usrB.getId()); logger.info("RoleID in
		 * USER_ROLE is " + usrRB.getRoleID() + " / " + usrRB.getUserRole());
		 **/
	 ...
	 * ModeAndView loginSuccess(HttpServletRequest request, HttpServletResponse
	 * respone) throws Exception {
	 * 
	 * Authentication auth = SecurityContextHolder.getContext().getAuthentication();
	...
	public String accessDenied(Model model, Principal principal) throws Exception {
		...
		if (principal != null) {
			model.addAttribute("message",
					"Hi " + principal.getName() + "<br> Bạn không có quyền truy cập vào trang này!");
		} /*
