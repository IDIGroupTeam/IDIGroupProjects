				...
				<c:forEach items="${chungTu.hangHoaDs}" var="hangHoa"
					...
					<tr id="hangTien${status.index}">
						...
						<td class="text-right"><c:choose>
							...
							&nbsp;${chungTu.loaiTien.maLt} <br />
									<fmt:formatNumber
										value="${hangHoa.tkThanhtoan.soTien.soTien*chungTu.loaiTien.banRa}"
										maxFractionDigits="2"></fmt:formatNumber>
							&nbsp;VND</c:otherwise>
							</c:choose></td>
					</tr>
			...
			<tbody>
				<c:forEach items="${chungTu.hangHoaDs}" var="hangHoa"
					...
					<tr id="thue${status.index}">
						...
						<td><fmt:formatNumber
								value="${hangHoa.tkThueXk.soTien.soTien}" maxFractionDigits="2"></fmt:formatNumber></td>
						<td>${hangHoa.tkThueXk.loaiTaiKhoan.maTk}</td>
						<%-- <td>${hangHoa.thueSuatGtgt}</td>
						<td><fmt:formatNumber
								value="${hangHoa.tkThueGtgt.soTien.giaTri}" maxFractionDigits="2"></fmt:formatNumber>
							VND</td>
