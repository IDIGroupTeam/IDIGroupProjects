	...
	$(function() {
		...
		function capNhatTongTienTxt() {
			...
			$("#soTien\\.giaTriQdTxt").html(
					accounting.formatNumber(tongGiaTri * tyGia, thapPhan, ",")
							+ " VND");
		}
		...
		function dangKySuKien() {
			...
			$("input[id^='taiKhoanCoDs']").click(function() {
				var tr = $(this).parents("tr");
				var tbody = tr.parent();
			...
			$("select[id^='taiKhoanCoDs']").click(function() {
				var tr = $(this).parents("tr");
				var tbody = tr.parent();
				tbody.find("tr.active").removeClass("active");
				tr.addClass("active");
		...
		function khoiTao() {
			...
			$("select[id^='taiKhoanCoDs'][id$='\\.loaiTaiKhoan\\.maTk']").each(
					function() {
						var maTkCo = $(this).val();
			...
			$("input[id^='taiKhoanCoDs'][id$='\\.soTien\\.soTien']").number(
					true, thapPhan);
...
<div class="row form-group">
	...
	<div class="col-sm-4">
		...
		<form:hidden path="taiKhoanNoDs[0].maNvkt" />
				...
				<tr id="${status.index}">
					<td class="text-left"><form:select cssClass="form-control"
							...
							<form:options items="${loaiTaiKhoanDs}" itemValue="maTk"
								itemLabel="maTenTk" />
						</form:select> <form:hidden path="taiKhoanCoDs[0].maNvkt" /> <form:hidden
							path="taiKhoanCoDs[${status.index}].soDu" /> <form:errors
							path="taiKhoanCoDs[${status.index}].loaiTaiKhoan.maTk"
							cssClass="error" /></td>
...
<div class="row form-group">
	<div class="col-sm-12">
		<a href="${url}/chungtu/phieuthu/danhsach" class="btn btn-info btn-sm">Danh
			sách phiếu thu</a> <a
			href="${url}/chungtu/phieuthu/xem/${mainFinanceForm.maCt}"
