...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#mainFinanceForm").attr("action", "${url}/chungtu/phieunhapkho/luu");
		...
		function thayDoiHangHoa() {
			alert("hi");
			var id = $(this).parrent().parrent().prop('id');
			alert("hi " + id);

			// Thay đổi đơn vị tính
			$("#nghiepVuKeToanDs[" + id + "]\\.hangHoa\\.donVi\\.tenDvTxt")
					.html();

			// Thay đổi đơn giá
			$("#nghiepVuKeToanDs[" + id + "]\\.donGia\\.soTien").html();

			// Xóa tổng tiền
			$("#nghiepVuKeToanDs[" + id + "]\\.tongTien").html("");

			// Xóa số lượng
			$("#nghiepVuKeToanDs[" + id + "]\\.soLuongTn").html("");
		...
		$("#themHh").click(
				function() {
					...
					$("input[id^='nghiepVuKeToanDs'][id$='\\.hangHoa\\.maHh']")
							.change(thayDoiHangHoa);
				});
		...
		function khoiTao() {
			...
			$("input[id^='nghiepVuKeToanDs'][id$='\\.hangHoa\\.maHh']").change(
					thayDoiHangHoa);
...
<div class="row form-group">
	<label class="control-label col-sm-2" for="khoBai.maKho">Kho</label>
	<div class="col-sm-4">
		<form:select path="khoBai.maKho" cssClass="form-control">
...
<div class="table-responsive row form-group">
	...
	<table id="taiKhoanTbl"
		...
		<tbody>
			<c:forEach items="${mainFinanceForm.nghiepVuKeToanDs}"
				var="nghiepVuKeToan" varStatus="status">
				<tr id="${status.index}">
					<td><form:select cssClass="form-control"
							path="nghiepVuKeToanDs[${status.index}].hangHoa.maHh">
							<form:option value="0" label=""></form:option>
							<form:options items="${hangHoaDs}" itemValue="maHh"
								itemLabel="tenHh"></form:options>
						</form:select></td>
					<td><form:select cssClass="form-control"
							path="nghiepVuKeToanDs[${status.index}].taiKhoanNo.loaiTaiKhoan.maTk">
							<form:option value="0" label=""></form:option>
							<form:options items="${loaiTaiKhoanDs}" itemValue="maTk"
								itemLabel="maTenTk"></form:options>
						</form:select></td>
					<td><form:select cssClass="form-control"
							path="nghiepVuKeToanDs[${status.index}].taiKhoanCo.loaiTaiKhoan.maTk">
							<form:option value="0" label=""></form:option>
							<form:options items="${loaiTaiKhoanDs}" itemValue="maTk"
								itemLabel="maTenTk"></form:options>
						</form:select></td>
					<td><span
						id="nghiepVuKeToanDs[${status.index}].hangHoa.donVi.tenDvTxt"></span></td>
					<td><form:input cssClass="form-control"
							path="nghiepVuKeToanDs[${status.index}].soLuongTn" /></td>
					<td><form:input cssClass="form-control"
							path="nghiepVuKeToanDs[${status.index}].donGia.soTien" /></td>
					<td><span id="nghiepVuKeToanDs[${status.index}].tongTien"></span></td>
				</tr>
			</c:forEach>
			<tr>
				<td colspan="7">
...
<div class="row form-group">
	<div class="col-sm-2">
		<a href="${url}/chungtu/phieunhapkho/danhsach"
